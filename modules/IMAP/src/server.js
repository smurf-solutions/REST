...
function IMAPServer(imapPort){
  ...
  net.createServer(function (socket) {
    ...
    if(DEBUG) console.log(" OPEN "+socket.name+"\r\n")
    ...
    socket.on('data', function (data) {
      ...
      if(processedCommand){
        ...
        if(IMAPCommands[processedCommand.command.command]){
			if(DEBUG) console.log(" "+processedCommand.command.command + " " + JSON.stringify(processedCommand.command.args) );
			if(DEBUG) console.dir(IMAPCommands[processedCommand.command.command]);
			
			IMAPCommands[processedCommand.command.command].callback(processedCommand.command, socket);
        } else {
			if(DEBUG) console.error("Unknown command '"+ processedCommand.command.command +"'");
        }
      }
    });
...
function ProcessCommand(data){
	...
	if(offset >= 0){
		...
		ret.command.args.forEach( function(v,i){
			ret.command.args[i] = v.replace(/^\"/,'').replace( /\"$/,'')
		})
		return ret
	}
	return null;
